...
def generate(args):
    ...
    with torch.no_grad():
        out_tensor, visu = model.restoration(
            ...
            ddim_num_steps=args.alg_palette_ddim_num_steps,
            ddim_eta=args.alg_palette_ddim_eta,
        )
        out_img = to_np(
            out_tensor
    ...
    if hasattr(args, "bbox_in") and args.bbox_in is not None:
    ...
    if hasattr(args, "dir_out") and args.dir_out is not None:
        logging.info(
        ...
        logging.info("Missing --dir-out option")
        return

    out_files = []
    if args.write:
        out_files.append(str(out_file.with_suffix(f".orig{out_suffix}")))
        out_files.append(str(out_file.with_suffix(f".cond{out_suffix}")))
        out_files.append(str(out_file.with_suffix(f".generated{out_suffix}")))
        out_files.append(str(out_file.with_suffix(f".y_t{out_suffix}")))
        ...
        cv2.imwrite(
            str(out_file.with_suffix(f".generated{out_suffix}")), out_img_real_size
        )
        cv2.imwrite(str(out_file.with_suffix(f".y_t{out_suffix}")), to_np(y_t))

        if mask is not None:
            out_files.append(str(out_file.with_suffix(f".y_0{out_suffix}")))
            out_files.append(str(out_file.with_suffix(f".generated_crop{out_suffix}")))
            out_files.append(str(out_file.with_suffix(f".mask{out_suffix}")))

            cv2.imwrite(
                str(out_file.with_suffix(f".y_0{out_suffix}")), to_np(img_tensor)
            )
            cv2.imwrite(
                str(out_file.with_suffix(f".generated_crop{out_suffix}")), out_img
            )
        ...
        if args.cond_in:
            ...
            out_files.append(str(out_file.with_suffix(f".orig_crop{out_suffix}")))
    ...
    logging.info(f"[8/%i] success - %s" % (PROGRESS_NUM_STEPS, " - ".join(out_files)))
...
def inference(args):
    ...
    if args.nb_samples > 1:
        for i in tqdm(range(args.nb_samples)):
            ...
            frame, lmodel, lopt = generate(args)
            args.lmodel = lmodel
            args.lopt = lopt
    else:
        frame, lmodel, lopt = generate(args)
